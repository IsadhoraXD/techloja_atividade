            ...
            <div class="p-5">
                <div class="flex justify-between items-start mb-2">
                    <div>
                        ...
                        <span class="text-xs px-3 py-1 bg-purple-300 text-purple-900 rounded-full">${produto.categoria}</span>
                    </div>
                    <div class="text-right">
                        <div class="font-bold text-xl">R$ ${produto.preco.toFixed(2).replace('.', ',')}</div>
                    </div>
                </div>
                <button onclick="adicionarAoCarrinho(${produto.id})"
                        class="mt-3 w-full bg-purple-900 hover:bg-purple-900 text-white py-3 rounded-2xl font-medium flex items-center justify-center gap-2"
...
function mostrarCarrinho() {
    ...
    if (carrinho.length === 0) {
        itensContainer.innerHTML = '<p class="text-center py-8 text-slate-500">Seu carrinho está vazio.</p>';
    } else {
        carrinho.forEach((item, index) => {
            total += item.preco;
            const itemHTML = `
                <div class="flex gap-4 border-b pb-4">
                    <img src="${item.imagem}" class="w-16 h-16 object-cover rounded-xl">
                    <div class="flex-1">
                        <div class="font-semibold">${item.nome}</div>
                        <div class="text-cyan-600">R$ ${item.preco.toFixed(2).replace('.', ',')}
                    </div>
                    </div>
                    <button onclick="removerDoCarrinho(${index})" class="text-red-500 hover:text-red-700"></button>
                </div>
            `;
